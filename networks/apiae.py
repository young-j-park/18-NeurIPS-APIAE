...
class APIAE(object):
    ...
    def __init__(self,R,L,K,dt,n_x,n_z,n_u,ur,lr,learn='af',useIC=True):   
        ...
        self.useIC = useIC # flag whether this network uses the nference network for control sequence
        ...
        # Initializing the tensorflow variables and saver
    ...
    def _create_network(self):
        ...
        self._create_network_woResample()
    ...
    def _create_network_woResample(self):
        # Initialize
        muhat = self.muhat
        Sighat = self.Sighat
        uffseq = self.uffhat
        ...
        for r in range(self.R):
            ...
            zseq, dwseqnew, alpha, S, bound, ess = self.Simulate(z0, muhat, Sighat, uffseq, dwseq, doResample=False)
            ...
            # Save variables
            self.apiae_list.append(bound)
            self.apiae_ess_hist.append(ess)
            
            if 'f' not in self.learn:
                self.museq_list.append(tf.reduce_sum(alpha*zseq, axis=1, keepdims=True))
                self.zseq_list.append(zseq)
    ...
    def _create_network_wResample(self):    
        ...
        for r in range(self.R):
            ...
            # Run dynamics and calculate the cost
            zseq, dwseqnew, alpha, S, bound, _ = self.Simulate(z0, muhat, Sighat, uffseq, dwseq, doResample=True)
            ...
            # Save variables
            self.apiaefivo_list.append(bound)
    ...
    def _create_loss_optimizer(self):
        self.ess_iwae = self.apiae_ess_hist[0]
        self.iwae = self.apiae_list[0]
        self.apiae = self.apiae_list[-1]
        self.fivo = self.apiaefivo_list[0]
        self.apiaefivo = self.apiaefivo_list[-1]
        
        # Use ADAM optimize 
        if self.learn == 'a':
            self.loss = -self.apiae
        elif self.learn == 'i':
            self.loss = -self.iwae
        elif self.learn == 'f':
            self.loss = -self.fivo
        elif self.learn == 'af':
            self.loss = -self.apiaefivo
    ...
    def Simulate(self, z0, muhat, Sighat, uffseq, dwseq, doResample = False):
    ...
    def partial_fit(self, X):
        ...
        opt, loss, museq_list, apiae, iwae, fivo, apiaefivo \
            = self.sess.run((self.opt, self.loss, self.museq_list, \
                             self.apiae, self.iwae, self.fivo, self.apiaefivo), feed_dict={self.xseq: X})
        return loss, museq_list, apiae, iwae, fivo, apiaefivo
